...
async function buildDashboardSummary(locationId) {
...
export const getDashboardSummary = async (req, res, next) => {
  try {
    ...
    const cacheKey = `dashboard:summary:${locationId || 'all'}`;
    const summary = await getOrSet(cacheKey, DASHBOARD_CACHE_TTL_SECONDS, () =>
      buildDashboardSummary(locationId)
    );
    res.json(summary);
  } catch (err) {
    next(err);
